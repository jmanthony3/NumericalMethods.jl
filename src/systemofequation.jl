...
function conjugate_gradient(
    ...
    if positive_definite(SOE.A)
        ...
        k, approximations, errors = 2, [x], [SOE.tol*10]
        while errors[end] > SOE.tol && k <= SOE.N
...
function steepest_descent(SOE, x::Vector{Float64})::Vector{Float64}
    k, approximations, errors = 2, [x], [SOE.tol*10]
    while errors[end] > SOE.tol && k <= SOE.N
        r = SOE.b - (SOE.A * x)
...
function solve(SOE::SystemOfEquation;
    ...
    C       ::Union{Nothing, Bool, Matrix{Float64}} = nothing)::Vector{Float64}
    return if method == :conjugate_gradient
        conjugate_gradient(SOE, x; C=C)
    elseif method == :steepest_descent
        steepest_descent(SOE, x)
    elseif method == :gaussian_elimination
